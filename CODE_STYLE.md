...
本文档基于当前仓库代码实测整理，主要目标是**记录现有风格**；同时补充少量“开发原则”（性能优先、代码可读性优先），用于新增/重构代码的决策参考。
...
- **代码可读性优先**：默认优先写清晰、可维护、易审查的代码；避免“聪明但难懂”的实现；必要时用中文注释说明意图与边界条件。
- **性能优先**：对压测门禁指标敏感/高频路径优先保证性能（减少不必要的拷贝、分配、锁竞争与阻塞）；性能优化必须用数据验证（至少跑一次 `scripts/pressure/pr_gate.ps1`）。
