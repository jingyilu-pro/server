...
ManagerService::ManagerService(const RuntimeConfig& config)
    : BasicHttpService("manager", config.server.manager)
{
    register_handler("/v1/route/login", [config](evhttp_request* request) {
        ...
        response.set_code(0);
        response.mutable_login_endpoint()->set_host(config.server.login.host);
        response.mutable_login_endpoint()->set_port(static_cast<uint32_t>(config.server.login.port));
